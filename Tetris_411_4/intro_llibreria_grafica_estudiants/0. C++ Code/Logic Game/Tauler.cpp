    ...
    for (int i = 0; i < figura.getNf() && trobat; i++) {
        for (int j = 0; j < figura.getNc() && trobat; j++) {
            if (forma[i][j] == 1) {
                ...
                if (!DinsDelTauler(figura, fila, columna) || !CelaBuida(figura, fila, columna)) {
                    trobat = false;
                }
            }
        }
...
void Tauler::esborraFigura(const Figura& figura) {

    for (int i = 0; i < figura.getNf(); i++) {
        for (int j = 0; j < figura.getNc(); j++) {
            if (figura.getValorCasella(j, i) != 0) {
                m_tauler[i + figura.getY()][j + figura.getX()] = COLOR_NEGRE;
...
void Tauler::colocarFigura(const Figura& figura) {
    ...
    for (int i = 0; i < figura.getNf(); i++) {
        for (int j = 0; j < figura.getNc(); j++) {
            if (figura.getValorCasella(j, i) != 0) {
                m_tauler[i + figura.getY() ][j + figura.getX()] = figura.getColorFigura();
            }
        }
    }
    figuraColocada = true;
